...
class Functions:
    ...
    def create_game_gui(self, lenght: int) -> List[str]:
        try:
    ...
    def init_game_session(self) -> None:
        ...
        session.setdefault(
            "visuals", ["_" if ch != " " else " " for ch in session["word_to_guess"]]
        )

    def handle_guess(self, player , guess: str) -> Optional[Union[str, None]]:
        # Normalize guess
        ...
        if (
            (not guess.isalpha())
            or (len(guess) == 1 and guess not in session["usable_letters"])
            or (len(guess) > 1 and guess != session["word_to_guess"])
        ):
            session["tries"] -= 1
            flash(
                "Invalid guess. Please use the provided letters or guess the word.",
                "danger",
            )
    ...
    def end_game(self, player, won: bool) -> None:
        ...
        if won:
            ...
            flash(
                f"Congratulations! You've guessed the word: {session['word_to_guess']}",
                "success",
            )
        else:
            db_crud.update_player_after_lost_game(
                player, correct_guesses, wrong_guesses
            )
